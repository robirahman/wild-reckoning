...
describe('WeatherSystem', () => {
  describe('computeWeatherPenalty', () => {
    it('returns no penalty for clear weather', () => {
      const weather: WeatherState = { type: 'clear', description: '', persistenceTurnsLeft: 1, intensity: 0.5 };
    ...
    it('returns no penalty for cloudy weather', () => {
      const weather: WeatherState = { type: 'cloudy', description: '', persistenceTurnsLeft: 1, intensity: 0.5 };
      const penalty = computeWeatherPenalty(weather);
      expect(penalty.weightChange).toBe(0);
      expect(penalty.statModifiers).toHaveLength(0);
    });
    ...
    it('applies weight loss and CLI modifier for blizzard', () => {
      const weather: WeatherState = { type: 'blizzard', description: '', persistenceTurnsLeft: 2, intensity: 1.0 };
      const penalty = computeWeatherPenalty(weather);
    ...
    it('applies weight loss and CLI modifier for heat wave', () => {
      const weather: WeatherState = { type: 'heat_wave', description: '', persistenceTurnsLeft: 1, intensity: 1.0 };
      const penalty = computeWeatherPenalty(weather);
    ...
    it('scales with intensity', () => {
      const low: WeatherState = { type: 'heat_wave', description: '', persistenceTurnsLeft: 1, intensity: 0.5 };
      const high: WeatherState = { type: 'heat_wave', description: '', persistenceTurnsLeft: 1, intensity: 1.0 };
    ...
    it('applies HOM modifier for drought', () => {
      const weather: WeatherState = { type: 'drought_conditions', description: '', persistenceTurnsLeft: 3, intensity: 0.8 };
      const penalty = computeWeatherPenalty(weather);
      expect(penalty.statModifiers).toHaveLength(1);
    ...
    it('applies CLI modifier for heavy rain with no weight change', () => {
      const weather: WeatherState = { type: 'heavy_rain', description: '', persistenceTurnsLeft: 1, intensity: 0.7 };
      const penalty = computeWeatherPenalty(weather);
      expect(penalty.weightChange).toBe(0);
      expect(penalty.statModifiers).toHaveLength(1);
      expect(penalty.statModifiers[0].stat).toBe(StatId.CLI);
    ...
    it('applies only weight loss for frost (no stat modifier)', () => {
      const weather: WeatherState = { type: 'frost', description: '', persistenceTurnsLeft: 1, intensity: 1.0 };
      const penalty = computeWeatherPenalty(weather);
    ...
    it('returns zero penalty for benign weather types', () => {
      ...
      for (const type of benignTypes) {
        const weather: WeatherState = { type, description: '', persistenceTurnsLeft: 1, intensity: 0.8 };
        const penalty = computeWeatherPenalty(weather);
        expect(penalty.weightChange).toBe(0);
        expect(penalty.statModifiers).toHaveLength(0);
      }
